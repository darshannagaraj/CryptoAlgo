...
def is_shooting_star(candle,candlestick_data) :
    ...
    return upper_shadow >= 1.85 * body_size and body_size >= lower_shadow * 2 and body_size > average_candle_size
...
def find_movement_based_on_time_frame(s,client,market_type, Scanned_all,wrapper_obj, specifictime,script,drop_rows=0 ):
    if s['symbol'] != script:
        return
    atr_period = 10
    atr_multiplier = 3.0
        ...
        if is_volume_greater_than_previous(pcandle, previous_candles):
            ...
            if boldifSati and is_outside_bollinger_upper(pcandle, pcandle['BollingerUpper']):
                ...
                if (is_shooting_star(JustCandle, previous_candles) or (is_candle_red(JustCandle, previous_candles))):
                    ...
                    sell = "yes"
            elif boldifSati and is_inside_bollinger_upper(pcandle, pcandle['BollingerUpper'],  pcandle['BollingerLower']):
    ...
    while True:
        try:
            ...
            for s in symbol_list['symbols']:
                try:
                    Future_scan =  find_movement_based_on_time_frame(s, client, "future", Scanned_all,Wrapper_obj, '2023-07-31 06:25:00','COMPUSDT')
